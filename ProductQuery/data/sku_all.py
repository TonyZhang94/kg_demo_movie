...
from ProductQuery.data.delete_all_data import *
from ProductQuery.data.add_data import *
...
def insert_model2cid(df_ori, cid, table="model_to_catalog"):
    ...
    df["cid"] = cid
    try:
        sql = f"DELETE FROM {table} WHERE cid = '{cid}';"
...
if __name__ == '__main__':
    ...
    for task in tasks:
        ...
        while True:
            try:
                insert_fixed_properties(copy.deepcopy(df))
                break
            except Exception as e:
                print(task, "try one time")
                if flag is True:
                    raise e
                flag = True
                continue
